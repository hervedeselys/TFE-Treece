...
if __name__ == "__main__":
    X = np.zeros((120,4))
    Y = np.zeros((120,))
    with open('Datasets/'+database+' - Learning Set.csv', 'rb') as iris:
    ...
    equations_info = []
    attributes_info = []
    with open(database+'_attribute_equation_info.csv', 'rb') as equations:
    ...
    if cut == 1:
        pl.savefig('all_'+database+'_plots_with_cuts.png')
        pl.savefig('all_'+database+'_plots_with_cuts.eps')
    elif cut == 0:
        pl.savefig('all_'+database+'_plots.png')
        pl.savefig('all_'+database+'_plots.eps')
    #pl.show()
